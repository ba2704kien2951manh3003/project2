...
tid_t process_execute(const char *file_name)
{
  ...
  /* Add */
  char *proc_name;  // var to get process name a.k.a the first argument
  char *save_ptr;
  ...
  // palloc page in user pool
  proc_name = palloc_get_page(0);
  if (proc_name == NULL) // make sure palloc success
    goto fail;
  proc_name = strtok_r(file_name, " ", &save_ptr); // parse file_name
  /* End */

  /* Create a new thread to execute FILE_NAME. */
  tid = thread_create(proc_name, PRI_DEFAULT, start_process, fn_copy); // parse process name
...
start_process(void *file_name_)
{
  ...
  /* if load success, set up stack; if not, exit */
  if (success)
...
void init_stack(int argc, char *argv[], void ** stackpointer)
{
  ASSERT(argc >= 0);
  ...
  for (int i = argc - 1; i >= 0; i--)
  {
    *stackpointer = *stackpointer - (strlen(argv[i]+1))*sizeof(char);
    ...
    address[i] = (uint32_t *) *stackpointer;
